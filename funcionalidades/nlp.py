...
class SummaryStruct(BaseModel):
    ...
    key_topics: list[str] = Field(description="Uma lista de 4 a 5 tópicos principais contidos nas avaliações.")
...
def extrair_sentimento(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop(columns=["sent_analysis_raw"])
...
@st.cache_resource
def processar_reviews(data: str) -> "SummaryStruct":
    """"""
